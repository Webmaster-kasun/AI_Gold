...
def run_bot():
    ...
    for name, config in ASSETS.items():
        ...
        # MERGED FIX: ATR-adaptive SL — replaces fixed 900–1100p range.
        # SL = ATR x 1.0, clamped to [700p, 1500p]. Scales with real volatility.
        if raw_atr and raw_atr > 0:
            stop_pips = max(700, min(int(raw_atr * 1.0), 1500))
            sl_source = "ATR x1.0"
        else:
            stop_pips = 1000
            sl_source = "fallback"
        log.info("SL: " + str(stop_pips) + "p (" + sl_source + ")")
        ...
        # TP: prefer R1/S1 structural target; default 1.5x SL.
        # R:R floor lowered 2.0 → 1.5 — gold's realistic intraday range
        # means 2:1 was blocking too many valid setups.
        tp_pips  = min(int(stop_pips * 1.5), 2500)
        tp_label = "1.5:1 R:R (default)"
        ...
        if cpr_levels and price:
            ...
            if target_level:
                ...
                if stop_pips * 1.5 <= dist <= stop_pips * 4:
                    tp_pips  = min(int(dist), 2500)
                    tp_label = ("R1=" + str(r1) if direction == "BUY" else "S1=" + str(s1)) + " (structural)"

        rr = tp_pips / stop_pips
        if rr < 1.5:
            scan_results.append(config["emoji"] + " " + name + ": R:R=" + str(round(rr, 1)) + " < 1:1.5 skip")
            continue
